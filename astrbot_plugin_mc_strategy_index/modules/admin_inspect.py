...
class AdminInspectModule:
    ...
    async def inspect_library(self, event: AstrMessageEvent):
        ...
        if index_updated:
            text += "索引图已自动更新。\n"
        if enable_edit and has_api_key:
            text += "发送 /整改攻略 <序号> 触发AI图片整改"
        else:
            text += "发送 /编辑攻略 <序号> 手动修正攻略信息"
        yield event.plain_result(text)
    ...
    async def rectify_strategy(self, event: AstrMessageEvent):
        ...
        if not self.plugin.config.get("enable_image_edit", False):
            yield event.plain_result("图片编辑功能未启用。请在插件配置中开启 enable_image_edit 并配置 image_api_key。")
            return

        yield event.plain_result(f"正在为「{strategy.get('name')}」调用图片编辑API...")

        new_image = await self.plugin.image_api.generate_index_image(strategy)
        if new_image:
            old_imgs = strategy.get("image_paths", [])
            ...
            yield event.plain_result("图片编辑API暂不可用或调用失败。")
        ...
        status_text = "已规范" if strategy["status"] == "normal" else "部分完善"
        yield event.plain_result(
            f"整改完成「{name}」\n"
            f"状态: {status_text}\n"
            f"分类: {strategy.get('category', '未分类')}\n"
            f"触发词: {', '.join(strategy.get('trigger_words', []))}\n"
            f"图片: {'已更新' if new_image else '未变更'}"
        )
    ...
    async def view_logs(self, event: AstrMessageEvent):
        ...
        for log in recent:
            ...
            text += f"  {log.get('detail', '')} | {log.get('operator', '')}\n"
        yield event.plain_result(text)
